...
class MLPSubstrate(DefaultSubstrate):
    ...
    def __init__(self, layers: List[int], coor_range: Tuple[float] = (-1, 1, -1, 1, -1, 1)):
        """
        layers: list of integers, the number of neurons in each layer
        coor_range: tuple of 6 floats, the range of the substrate. (x_min, x_max, y_min, y_max, z_min, z_max)
        """
        ...
        assert coor_range[4] < coor_range[5], "z_min should be less than z_max"
        num_inputs = layers[0]
